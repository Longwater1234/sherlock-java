...
public class SearchProcessor implements Runnable {
  ...
  @Override
  public void run() {
    HttpRequest request = HttpRequest.newBuilder().uri(URI.create(finalUri)).header("Content-Type", "application/json")
    ...
    try {
      ...
      System.err.printf("[!] failed at %s %s%n", service, ANSI_RESET);
    }
  }
}